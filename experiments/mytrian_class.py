# -*- coding: utf-8 -*-
"""
Created on Wed Feb 12 13:49:53 2020
...
def options(argv=None):
    ...
    # required.
    parser.add_argument('-o', '--outfile',default='./results/my_classifier_0330 ', type=str,
                        metavar='BASENAME', help='output filename (prefix)') # result: ${BASENAME}_feat_best.pth
    parser.add_argument('-i', '--dataset-path',default='D:/github/dataset',type=str,
                        metavar='PATH', help='path to the input dataset') # like '/path/to/ModelNet40'
    parser.add_argument('-c', '--categoryfile',default='./sampledata/teeth.txt', type=str,
                        metavar='PATH', help='path to the categories to be trained') # eg. './sampledata/modelnet40_half1.txt'

    # settings for input data
    parser.add_argument('--dataset-type', default='teeth',
    ...
    parser.add_argument('-l', '--logfile', default='./results/my_classifier_0330.log ', type=str,
...
#    
#    x_RandomUnderSample_resampled,y_RandomUnderSample_resampled = model_RandomUnderSampler.fit_sample(trainset[:-1],trainset[-1])         #输入数据并进行欠抽样处理
...
#    tr_RandomUnderSampler_resampled = pd.concat([x_RandomUnderSample_resampled,y_RandomUnderSample_resampled],axis=1)
#    
#    x_RandomUnderSample_resampled,y_RandomUnderSample_resampled = model_RandomUnderSampler.fit_sample(trainset[:-1],trainset[-1])         #输入数据并进行欠抽样处理
#    x_RandomUnderSample_resampled = pd.DataFrame(x_RandomUnderSample_resampled,columns=['col1','col2','col3','col4','col5'])
#    y_RandomUnderSample_resampled = pd.DataFrame(y_RandomUnderSample_resampled,columns=['label'])
#    te_RandomUnderSampler_resampled = pd.concat([x_RandomUnderSample_resampled,y_RandomUnderSample_resampled],axis=1)
#    
#
#    # training
#    
#    act = Action(args, num_classes)
#    run(args, tr_RandomUnderSampler_resampled, te_RandomUnderSampler_resampled, act)
...
#                ptlk.data.transforms.VoxelGridfilter(),\
        #        ptlk.data.transforms.RandomRotatorZ(),\
         #       ptlk.data.transforms.RandomJitter()\
            ])
    #print("cinfo:",cinfo)
    dataset = ptlk.data.datasets.Teeth(args.dataset_path, transform=transform, classinfo=cinfo)
